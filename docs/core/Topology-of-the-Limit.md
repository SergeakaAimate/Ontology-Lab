...
Four boundary types are identified—not as flaws, but as features:
- **Reflexive** (`P-9: Reflexivity`) → infinite recursion → `W₄` as self-application regime  
- **Operational** (`P-34: Bindability / Γ`) → resolved via external agent (observer)  
- **Epistemic** (`P-8: Boundedness + P-35: Capacity`) → finite form channels infinite content  
- **Communicative** (`P-27: Interpretability`) → complexity adaptation via dialogic interface  

The result: *the system’s boundedness becomes the form of its infinity*.
